...
selection=$(echo -ne "$final_rofi" | rofi -markup-rows -theme ~/.config/rofi/Notif.rasi -dmenu -eh 3 -a "$active_list" -u "$urgent_list" -sep '\x0f' -p "                  Notifications" -no-fixed-num-lines -lines 12 -i -no-config)
